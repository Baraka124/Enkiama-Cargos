...
// this driver's OWN id — so we only ever show parcels assigned to them
const myDriverId = ref(null)
const appStatus = ref(null)  // { status, carrier, note, requested_carrier }
async function loadMyDriverId() {
  try { const { data } = await supabase.rpc('my_driver_profile'); myDriverId.value = data?.id || null } catch (e) {}
  try { const { data } = await supabase.rpc('my_application_status'); appStatus.value = data || null } catch (e) {}
}
// this driver's assigned parcels — MUST match their driver_id, not just the stage
const mine = computed(() => consignments.value.filter(p =>
  ['with_driver','delivered','failed'].includes(p.stage) &&
  (myDriverId.value ? p.driverId === myDriverId.value : false)))
...
function renderMap() {
  ...
  stops.forEach((p, i) => {
    ...
    pts.push(ll)
    const icon = L.divIcon({ html: `<div style="background:${carrier.value?.accent||'#3E5BD6'};color:#fff;width:26px;height:26px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12.5px;box-shadow:0 2px 8px rgba(0,0,0,.5)"><span style="transform:rotate(45deg)">${i+1}</span></div>`, className:'', iconSize:[26,26], iconAnchor:[13,26] })
...
onMounted(async () => { spaceLoading.value = true; await loadMyDriverId(); await fetchAll(); await loadAvailable(); await loadSpace(); spaceLoading.value = false; subscribe(); await nextTick(); renderMap(); startLocationSharing() })

// #18 — share live location while on a run (privacy: only reported, receiver sees it only while with-driver)
let locWatch = null
function startLocationSharing() {
  if (!navigator.geolocation || locWatch) return
  locWatch = navigator.geolocation.watchPosition(
    async (pos) => {
      try { await supabase.rpc('report_location', {
        p_lat: pos.coords.latitude, p_lng: pos.coords.longitude, p_heading: pos.coords.heading || null }) } catch (e) {}
    },
    () => {},   // silently ignore if denied — tracking still works without live GPS
    { enableHighAccuracy: true, maximumAge: 30000, timeout: 20000 }
  )
}
onUnmounted(() => { if (locWatch) navigator.geolocation.clearWatch(locWatch) })
  ...
  <div class="wrap">
    ...
    <div v-if="!spaceLoading && appStatus && appStatus.status !== 'active'" class="drv-gate">
      ...
      <div v-else-if="appStatus.status === 'rejected'" class="drv-gate-card">
        ...
        <p v-if="appStatus.note">{{ appStatus.note }}</p>
        <p v-else>Your application wasn't approved this time. You can review your details and apply again.</p>
        <RouterLink to="/driver/apply" class="btn btn-accent">Apply again</RouterLink>
      </div>
    </div>
    <div v-else-if="!spaceLoading && !appStatus" class="drv-gate">
      <div class="drv-gate-card">
        <div class="drv-gate-ic"><Icon name="bike" :size="30" /></div>
        <h2>Join a carrier to start</h2>
        <p>To drive on Enkiama, apply to a carrier and verify your identity. Once approved, you'll see parcels and can start earning.</p>
        <RouterLink to="/driver/apply" class="btn btn-accent">Apply to a carrier</RouterLink>
      </div>
    </div>
    ...
    <div v-else-if="space" class="statrow">
      <StatCard :label="t('activeTasks')" :value="space.active_tasks" icon="box" tone="accent" />
      <StatCard :label="t('delivered')" :value="space.delivered_total" icon="check" tone="go" />
      <StatCard :label="t('feesEarned')" :value="fmtTZS(space.fees_earned)" icon="cash" tone="muted" />
      <StatCard :label="t('cashToRemit')" :value="fmtTZS(space.cash_to_remit)" icon="cash" :tone="space.cash_to_remit>0 ? 'owed' : 'muted'" :alert="space.cash_to_remit>0" />
    </div>
    ...
    <EmptyState v-else icon="truck" :title="t('noPendingStops')" hint="When dispatch hands you a consignment, it appears here for your run." />
