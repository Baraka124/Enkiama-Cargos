...
function matchesSearch(p) {
  ...
  if (!q) return true
  // status keyword search: "delivered", "on road", "cash", "owed", etc.
  const statusText = [p.stage, p.payMode, p.payState].filter(Boolean).join(' ').toLowerCase()
  if (statusText.includes(q)) return true
  // field search across all the useful columns
  return [p.code, p.receiver, p.receiverPhone, p.sender, p.senderPhone, p.addr, p.item, p.driverName]
...
async function copyTrackLink() {
  try { await navigator.clipboard.writeText(trackUrl.value); copied.value = true; setTimeout(()=>copied.value=false, 2000) } catch (e) { toast("Couldn't copy — long-press to copy manually", "warn") }
}
...
// driver applications awaiting this carrier's review
const driverRequests = ref([])
async function loadDriverRequests() {
  try { const { data } = await supabase.rpc('carrier_driver_requests'); driverRequests.value = data?.requests || [] } catch (e) {}
}
async function viewDoc(path) {
  if (!path) { toast('No document uploaded', 'warn'); return }
  try {
...
async function reviewDriver(id, decision) {
  try {
    const { data } = await supabase.rpc('review_driver_application', { p_driver_id: id, p_decision: decision })
    if (data?.ok) { toast(decision==='active' ? 'Driver approved' : 'Application rejected', 'ok'); await loadDriverRequests(); await loadDrivers() }
...
onMounted(async () => {
  ...
  try { await loadDriverRequests() } catch (e) {}
...
// #16 — export the ledger to CSV (for accountants, banks, records)
function exportLedgerCSV() {
  const rows = consignments.value || []
  if (!rows.length) { toast('Nothing to export yet', 'warn'); return }
  const headers = ['Code','Status','Sender','Receiver','Destination','Item','Weight (kg)','Delivery fee','Payment mode','Payment state','COD amount','Created']
  const esc = (v) => { const s = String(v ?? ''); return /[",\n]/.test(s) ? '"'+s.replace(/"/g,'""')+'"' : s }
  const lines = [headers.join(',')]
  for (const p of rows) {
    lines.push([p.code, p.stage, p.sender, p.receiver, p.addr, p.item, p.weight, p.fee, p.payMode, p.payState, p.cod, p.createdAt ? new Date(p.createdAt).toLocaleDateString() : ''].map(esc).join(','))
  }
  const blob = new Blob([lines.join('\n')], { type: 'text/csv;charset=utf-8;' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = `enkiama-ledger-${new Date().toISOString().slice(0,10)}.csv`
...
async function doRemit(p) {
  askConfirm('Confirm cash remitted?',
    ...
    async () => {
      ...
      catch (e) { toast(humanError(e, 'Remit failed'), 'warn') }
...
async function doMomo(p) {
  askConfirm('Mark paid by mobile money?',
    ...
    async () => {
      ...
      catch (e) { toast(humanError(e, 'Could not confirm'), 'warn') }
      actionBusy.value = false
    })
}
  ...
  <div class="topbar"><div class="inner">
    ...
    <div class="tb-actions-desktop">
      ...
      <div class="tb-icongroup">
        ...
        <button class="tb-ico" aria-label="Language" :title="isSwahili ? 'Switch to English' : 'Badilisha kwa Kiswahili'" @click="setLang(isSwahili?'en':'sw')"><span class="tb-ico-lang">{{ isSwahili ? 'EN' : 'SW' }}</span></button>
  ...
  <div class="wrap">
    ...
    <div class="statrow">
      <button class="statcard clickable" @click="tab='ledger'">
        ...
        <div class="statcard-body"><div class="statcard-v">{{ active.length }}</div><div class="statcard-l">{{ t('activeConsignments') }}</div></div>
      </button>
      <button class="statcard clickable" :class="{alert: actionList.length}" @click="tab='action'">
        ...
        <div class="statcard-body"><div class="statcard-v">{{ actionList.length }}</div><div class="statcard-l">{{ t('needsAction') }}</div></div>
      </button>
      <button class="statcard clickable" @click="tab='action'">
        <div class="statcard-ic owed"><Icon name="cash" :size="18" /></div>
        <div class="statcard-body"><div class="statcard-v owed-ink"><Money :amount="owedTotal" /></div><div class="statcard-l">{{ t('cashToCollect') }}</div></div>
      </button>
    ...
    <div class="dtabs" style="align-items:center">
      <button class="dtab" :class="{on:tab==='action'}" @click="tab='action'">{{ t('needsAction') }} <span class="tb-count">{{ actionList.length }}</span></button>
    ...
    <!-- ACTION -->
    <div v-if="tab==='action'">
      <div class="fg fg-wide" style="margin-bottom:12px;position:relative">
    ...
    <div v-else-if="tab==='ledger'">
      <div class="ledger-toolbar">
        <div class="fg fg-wide" style="flex:1;margin-bottom:0;position:relative">
          <input v-model="search" placeholder="Search code, receiver, phone, address…" style="padding-left:38px" />
          <Icon name="search" :size="16" style="position:absolute;left:13px;top:13px;color:var(--ink-faint)" />
        </div>
        <button class="btn btn-ghost" @click="exportLedgerCSV"><Icon name="send" :size="15" /> Export CSV</button>
      </div>
    ...
    <div v-else-if="tab==='team'">
      <!-- driver applications awaiting review -->
      <div v-if="driverRequests.length" class="panel dr-panel">
        <h2>Driver requests <span class="tb-count owed">{{ driverRequests.length }}</span></h2>
        <div class="sub">People who applied to drive for {{ carrier?.name }}. Review their identity documents before approving.</div>
        <div class="dr-list">
          <div v-for="d in driverRequests" :key="d.id" class="dr-card">
            <div class="dr-top">
              <div><b>{{ d.name }}</b><span class="dr-meta">{{ d.phone }} · {{ d.vehicle || 'vehicle n/a' }}<template v-if="d.license_no"> · Licence {{ d.license_no }}</template><template v-if="d.national_id_no"> · ID {{ d.national_id_no }}</template></span></div>
              <span class="dr-when">{{ d.applied_at ? new Date(d.applied_at).toLocaleDateString() : '' }}</span>
            </div>
            <div class="dr-docs">
              <button class="dr-doc" @click="viewDoc(d.license_photo_url)"><Icon name="camera" :size="14" /> View licence</button>
              <button class="dr-doc" @click="viewDoc(d.national_id_photo_url)"><Icon name="camera" :size="14" /> View national ID</button>
            </div>
            <div class="dr-actions">
              <button class="btn btn-accent" @click="reviewDriver(d.id, 'active')"><Icon name="check" :size="14" /> Approve</button>
              <button class="btn btn-ghost" @click="reviewDriver(d.id, 'rejected')">Reject</button>
            </div>
          </div>
        </div>
      </div>
