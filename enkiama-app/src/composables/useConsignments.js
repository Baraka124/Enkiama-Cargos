...
function subscribe() {
  ...
  channel = supabase.channel('cargos-live')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'consignment' }, () => { realtimeOk = true; fetchAll() })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'payment' }, () => { realtimeOk = true; fetchAll() })
    .subscribe((status) => { if (status === 'SUBSCRIBED') realtimeOk = true })
  // Fallback poll — only fires if realtime ISN'T delivering, and at a gentle 20s
  // to spare mobile data. When realtime works, this effectively never fetches.
  if (pollTimer) clearInterval(pollTimer)
  pollTimer = setInterval(() => {
    if (document.visibilityState !== 'visible') return
    if (realtimeOk) return              // realtime is handling updates — skip the poll
    fetchAll()
  }, 20000)
}
...
async function assignDriver(p, driverId) {
  // optimistic: reflect assignment immediately
  const target = consignments.value.find(c => c.id === p.id)
  const prev = target ? { stage: target.stage, driverId: target.driverId } : null
  if (target) { target.stage = 'with_driver'; target.driverId = driverId }
  const { data, error } = await supabase.rpc('assign_driver_to_parcel', { p_code: p.code, p_driver_id: driverId })
  if (error || (data && !data.ok)) {
    if (target && prev) { target.stage = prev.stage; target.driverId = prev.driverId }  // rollback
